...
router.get('/', verifyToken, async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM tauxTva WHERE actif = TRUE ORDER BY nom ASC');
...
router.post('/', verifyToken, requireRole('manager'), async (req, res) => {
    try {
        ...
        const existingCount = await pool.query('SELECT COUNT(*) FROM tauxTva WHERE actif = TRUE');
        const isFirst = parseInt(existingCount.rows[0].count) === 0;

        const result = await pool.query(
            'INSERT INTO tauxTva (nom, taux, is_default) VALUES ($1, $2, $3) RETURNING *',
            [nom, taux, isFirst]
        );
...
router.delete('/:id', verifyToken, requireRole('manager'), async (req, res) => {
    try {
        const result = await pool.query('UPDATE tauxTva SET actif = FALSE WHERE id = $1 RETURNING *', [req.params.id]);
        ...
        if (result.rows[0].is_default) {
            await pool.query('UPDATE tauxTva SET is_default = TRUE WHERE id = (SELECT id FROM taxe WHERE actif = TRUE LIMIT 1)');
        }
